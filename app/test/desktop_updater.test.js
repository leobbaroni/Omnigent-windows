...
describe("desktop_updater — pure helpers", () => {
  ...
  it("uses the production update endpoint in development", () => {
    ...
    // [win] This build publishes to GitHub Releases (provider: github), not a
    // generic URL feed, so the dev config must name the same provider/repo.
    const publish = packageConfig.build.publish[0];
    assert.equal(devConfig.provider, publish.provider);
    if (publish.provider === "github") {
      assert.equal(devConfig.owner, publish.owner);
      assert.equal(devConfig.repo, publish.repo);
    } else {
      assert.equal(devConfig.url, publish.url);
