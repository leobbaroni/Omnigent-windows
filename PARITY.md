...
| Claude Code inside **WSL** (slash commands, skills/plugins/connectors, AskUserQuestion) | The app starts the server and hosts the machine inside WSL, and the host reports `native-claude: needs-auth` until Claude is signed in inside the distro. That sign-in is an interactive browser login (`wsl -d Ubuntu -- claude`, then `/login`) which the tester cannot perform on the user's behalf, so the in-session behaviour was **not verified here**. It is Omnigent's Linux code path, unchanged by this shell. |
